...
const GlobalEconomyDashboard = () => {
  ...
  const fetchWorldBankIndicator = async (countryCode: string, indicator: string): Promise<IndicatorData> => {
    const response = await fetch(
      `https://api.worldbank.org/v2/country/${countryCode}/indicator/${indicator}?format=json`
    );
    const payload = await response.json();
    const rows = payload?.[1] ?? [];
    const latest = rows.find((entry) => entry.value !== null);
    return latest
      ? { value: latest.value, date: latest.date }
      : { value: null, date: null };
  };

  const fetchEurostatInflation = async (geoCode: string): Promise<IndicatorData> => {
    const response = await fetch(
    ...
    const latestTime = timeKeys.sort().at(-1);
      ...
      try {
        ...
        const results = await Promise.all(
          countries.map(async (country) => {
            const [gdp, growth, inflation, eurostatInflation] = await Promise.all([
              fetchWorldBankIndicator(country.wbCode, 'NY.GDP.MKTP.CD'),
              fetchWorldBankIndicator(country.wbCode, 'NY.GDP.MKTP.KD.ZG'),
              fetchWorldBankIndicator(country.wbCode, 'FP.CPI.TOTL.ZG'),
              fetchEurostatInflation(country.eurostatCode),
            ]);
